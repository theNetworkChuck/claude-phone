...
export function generateEnvFile(config) {
  ...
  const lines = [
    ...
    '',
    '# Google Cloud (Gemini LLM + Speech-to-Text)',
    `GEMINI_API_KEY=${config.api.gemini.apiKey}`,
    `GOOGLE_APPLICATION_CREDENTIALS=${config.api.google.credentialsPath}`,
    `GOOGLE_CLOUD_PROJECT=${config.api.google.projectId}`,
    '',
